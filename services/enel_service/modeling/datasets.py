...
from services.enel_service.common.configuration import GeneralSettings
from services.enel_service.common.db_schemes import JobExecutionModel, ApplicationExecutionModel
from services.enel_service.modeling.transforms import CustomData, CustomCompose, TransformationHandler
...
class ExecutionSubset(InMemoryDataset):
    ...
    def process(self):
        data_model_list: List[Union[ApplicationExecutionModel, JobExecutionModel]] = [
            torch.load(os.path.join(self.raw_dir, file_name), weights_only=False)
            for file_name in self.raw_file_names]
