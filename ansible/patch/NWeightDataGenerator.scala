...
object NWeightDataGenerator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length == 3) {
      ...
      println(s"Model Path: $modelPath is ignored. We load the nweight-user-features as resource file.")
    ...
    val model: MatrixFactorizationModel = loadModel(sc, numPartitions)
